...
void __iomem *esa_dma_get_mem(void)
{
	return si.dma_param;
}

int esa_dma_open(void)
{
	int ch;

	spin_lock(&si.lock);
	for (ch = DMA_CH_BEGIN; ch < DMA_CH_MAX; ch++) {
		if (!si.dma_ch[ch])
			break;
	}
	spin_unlock(&si.lock);
...
int esa_dma_set_callback(void (*cb)(void *data), void *param, int ch)
{
	si.dma_cb[ch] = cb;
	si.dma_cb_param[ch] = param;
