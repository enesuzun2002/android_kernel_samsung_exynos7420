...
/* if SEC_HWCNT enabled, it will be moved to vinstr.h */
enum {
    SHADER_HWCNT_BM,
    TILER_HWCNT_BM,
    MMU_L2_HWCNT_BM,
...
struct kbase_vinstr_client *kbasep_vinstr_attach_client_sec(
		...
		u32 bitmap[4], void *argp, void *kernel_buffer)
{
	return kbasep_vinstr_attach_client(vinstr_ctx, buffer_count, bitmap, argp, NULL);
}
void kbase_vinstr_detach_client_sec(struct kbase_vinstr_client *cli)
{
	kbase_vinstr_detach_client(cli);
}
