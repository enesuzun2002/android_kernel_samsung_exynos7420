 ...
 *
 * Caller must not hold runpool_irq.lock
 *
 * Return: true if caller should call kbase_backend_ctx_count_changed()
 */
static bool kbase_js_ctx_list_add_pullable(struct kbase_device *kbdev,
						...
						int js)
{
	...
	ret = kbase_js_ctx_list_add_pullable_nolock(kbdev, kctx, js);
 ...
 *
 * Variant of kbase_js_ctx_list_add_pullable_head()
 * where the caller must hold runpool_irq.lock
 *
 * @kbdev:  Device pointer
 * @kctx:   Context to add to queue
 * @js:     Job slot to use
		...
		struct kbasep_js_atom_retained_state *katom_retained_state)
{
	...
	if (kctx_as_nr == KBASEP_AS_NR_INVALID)
