...
static void dirty_hash(struct bio *bio, int error)
{
	 ...
	 * assumption.
	 */
	for(i = 0; i < (io->size >> v->data_dev_block_bits); i++) {
		/* Loop over all blocks */
		
		verity_hash_at_level(v, io->block + (sector_t)i, 0, &hash_block, &offset);
...
static int dirty_map(struct dm_target *ti, struct bio *bio)
{
	 ...
	 * away. 
	 */
	if (bio_data_dir(bio) != WRITE || bio->bi_size == 0) 
		goto process_bio;
