...
static int max86900_regulator_onoff(struct max86900_device_data *data, int onoff)
{
	...
	data->vdd_1p8 = regulator_get(NULL, "V_HRM_1.8V");
	if (IS_ERR(data->vdd_1p8)) {
		pr_err("%s - regulator_get fail\n", __func__);
		goto err_1p8;
	}

	data->vdd_3p3 = regulator_get(NULL, "V_HRM_3.3V");
...
int max86900_probe(struct i2c_client *client, const struct i2c_device_id *id )
{
	...
	err = sensors_create_symlink(&data->hrm_input_dev->dev.kobj, data->hrm_input_dev->name);
	if (err < 0) {
...
err_sysfs_create_group:
	sensors_remove_symlink(&data->hrm_input_dev->dev.kobj,data->hrm_input_dev->name);
