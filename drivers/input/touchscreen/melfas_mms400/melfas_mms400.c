	...
	if (info->lowpower_mode && info->ic_status >= LPM_RESUME) {
		...
		if (mms_i2c_read(info, wbuf, 2, rbuf, 1)) {
			input_err(true, &client->dev,
				  "%s [ERROR] Read AOT event info\n", __func__);
			goto ERROR;
		}

		input_info(true, &info->client->dev,
			   "%s start, event:%x! gpio:%d\n", __func__, rbuf[0],
			   gpio_get_value(info->dtdata->gpio_intr));
		...
		if (alert_type & MMS_LPM_FLAG_SPAY) {
			...
			input_info(true, &client->dev,
				   "%s - double tap event(%x, %x, %x, %x)",
				   __func__, rbuf[0], rbuf[1], rbuf[2],
				   rbuf[3]);

			info->scrub_x =
				((rbuf[0] & 0xFF) << 0) | ((rbuf[1] & 0xFF) << 8);
			info->scrub_y =
				((rbuf[2] & 0xFF) << 0) | ((rbuf[3] & 0xFF) << 8);
#ifdef CONFIG_SAMSUNG_PRODUCT_SHIP
			input_info(true, &client->dev, "%s: aod: %d\n",
				   __func__, info->scrub_id);
#else
			input_info(true, &client->dev, "%s: aod: %d, %d, %d\n",
				   __func__, info->scrub_id, info->scrub_x,
				   info->scrub_y);
#endif

		} else {
			input_err(true, &client->dev,
				  "%s [ERROR] Read a wrong aot action %x\n",
				  __func__, alert_type);
			return IRQ_HANDLED;
		}
