...
#define TC300K_FW_PATH_FFU	"ffu_tk.bin"
...
struct tc300k_data {
	...
	bool flip_mode;
	bool tc300k_power_enabled;
	bool tc300k_keyled_enabled;
};
			...
			struct tc300k_platform_data *pdata)
{
	...
	if (of_property_read_string(np, "coreriver,regulator_ic", &pdata->regulator_ic)) {
		input_err(true, dev, "[TK] %s Failed to get regulator_ic name property\n",__func__);
		return -EINVAL;
	}

	if (of_property_read_string(np, "coreriver,regulator_led", &pdata->regulator_led)) {
		input_err(true, dev, "[TK] %s Failed to get regulator_led name property\n",__func__);
	...
	if (of_property_read_u32(np, "coreriver,tsk_ic_num", &pdata->tsk_ic_num) < 0){
		input_info(true, dev, "[TK] %s Failed to get tsk_ic_num, TSK IC is TC300K\n", __func__);
	} else {
		if (pdata->tsk_ic_num == TC350K_TSK_IC)
			input_info(true, dev, "[TK] %s TSK IC is TC350K_TSK_IC[%d]\n", __func__, pdata->tsk_ic_num);
		else
			input_err(true, dev, "[TK] %s TSK IC is unknown![%d]\n", __func__, pdata->tsk_ic_num);
	}

	input_info(true, dev, "[TK] %s : %s, scl:%d, sda:%d, irq:%d, "
			"fw_name:%s, sensing_ch:%d, tkey_led:%d, tsk_ic:%d,%s\n",
			__func__, pdata->use_bitmap ? "Use Bit-map" : "Use OLD",
			pdata->gpio_scl, pdata->gpio_sda, pdata->gpio_int,
			pdata->fw_name, pdata->sensing_ch_num, pdata->touchkey_led,
			pdata->tsk_ic_num,
			(pdata->tsk_ic_num == TC350K_TSK_IC) ? "TC350K" : "TC300K");
	return 0;
}
#else
static int tc300k_parse_dt(struct device *dev,
...
int tc300k_touchkey_power(void *tdata, bool on)
{
	struct tc300k_data *data = (struct tc300k_data *)tdata;
	struct regulator *regulator;
	int ret = 0;

	if (data->tc300k_power_enabled == on)
		return 0;

	input_info(true, &data->client->dev, "%s %s\n", __func__, on ? "on" : "off");

	regulator = regulator_get(NULL, data->pdata->regulator_ic);
	if (IS_ERR(regulator)){
		input_err(true, &data->client->dev, "[TK] %s: regulator_ic get failed\n", __func__);
		return -EIO;
	}
	...
	if (on) {
		...
		if (ret) {
			input_err(true, &data->client->dev, "[TK] %s: regulator_ic enable failed\n", __func__);
			return ret;
		}
	} else {
		if (regulator_is_enabled(regulator)){
			...
			if (ret) {
				input_err(true, &data->client->dev, "[TK] %s: regulator_ic disable failed\n", __func__);
				return ret;
			}
		}
		else
	...
	data->tc300k_power_enabled = on;

	return 0;
}

int tc300k_touchkey_led_control(void *tdata, bool on)
{
	struct tc300k_data *data = (struct tc300k_data *)tdata;
	struct regulator *regulator;
	int ret=0;

	if (data->tc300k_keyled_enabled == on)
		return 0;

	if (!data->pdata->regulator_led)
		return 0;

	input_info(true, &data->client->dev, "%s %s\n", __func__, on ? "on" : "off");

	regulator = regulator_get(NULL, data->pdata->regulator_led);
	if (IS_ERR(regulator)){
		input_err(true, &data->client->dev, "[TK] %s: regulator_led get failed\n", __func__);
		return -EIO;
	}
	...
	if (on) {
		ret = regulator_enable(regulator);
		if (ret) {
			input_err(true, &data->client->dev, "[TK] %s: regulator_led enable failed\n", __func__);
			return ret;
		}
	} else {
		if (regulator_is_enabled(regulator))
			...
			if (ret) {
				input_err(true, &data->client->dev, "[TK] %s: regulator_led disable failed\n", __func__);
				return ret;
			}
			else
				regulator_force_disable(regulator);
...
static irqreturn_t tc300k_interrupt(int irq, void *dev_id)
{
	...
	for (i = 0 ; i < data->key_num * 2 ; i++){
		...
		if (key_handle_flag){
			...
			input_info(true, &client->dev, "[TK] key %s ver0x%02x\n",
	 ...
	 struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(data->pdata->touchkey_led != true){
		input_err(true, &client->dev, "[TK] %s: touchkey_led false\n", __func__);
		return count;
		}
...
static int load_fw_in_ffu(struct tc300k_data *data)
{
	struct i2c_client *client = data->client;
	int ret;

	ret = request_firmware(&data->fw, TC300K_FW_PATH_FFU, &client->dev);
	if (ret) {
		input_err(true, &client->dev, "[TK] %s fail(%d)\n", __func__, ret);
		return -1;
	}
...
static int tc300k_verify_fw(struct tc300k_data *data)
{
	...
	while (addr < data->fw_img->fw_len) {
		if ((addr % 0x40) == 0)
			input_info(true, &client->dev, "[TK] fw verify addr = %#x\n", addr);
...
static int tc300k_fw_update(struct tc300k_data *data, u8 fw_path, bool force)
{
	...
	if (fw_path == FW_INKERNEL) {
		...
		ret = load_fw_in_ffu(data);
		if (ret)
			return -1;

		data->fw_ver_bin = data->fw_img->first_fw_ver;
		if (!force && (data->fw_ver >= data->fw_ver_bin)) {
			input_info(true, &client->dev, "[TK] ffu: do not need firm update (IC:0x%x, BIN:0x%x)\n",
				data->fw_ver, data->fw_ver_bin);
			t300k_release_fw(data, fw_path);
			return 0;
		}
	...
	while (retry--) {
		...
		if (ret) {
			input_err(true, &client->dev, "[TK] %s tc300k_flash_fw fail (%d)\n",
				__func__, retry);
			continue;
		}
		...
		if (ret) {
			input_err(true, &client->dev, "[TK] %s tc300k_get_fw_version fail (%d)\n",
				__func__, retry);
			continue;
		}
...
i2c_pinctrl_get:
	...
	input_err(true, &data->client->dev, "%s: use dev's parent\n", __func__);
		...
		const struct i2c_device_id *id)
{
	...
	input_info(true, &client->dev, "%s [TK]\n", __func__);
	...
	if (data->pdata == NULL) {
		input_err(true, &data->client->dev, "[TK] failed to get platform data\n");
		ret = -EINVAL;
	...
	if (ret)
		input_err(true, &client->dev, "[TK] Failed to create sysfs group\n");
...
#if !defined(FEATURE_GRIP_FOR_SAR)
