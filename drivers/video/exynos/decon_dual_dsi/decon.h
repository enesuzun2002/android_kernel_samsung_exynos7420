...
struct exynos_decon_platdata {
	...
	enum decon_output_type	out_type[2];
	int	max_win;
	int	default_win;
};
...
void DISP_SS_EVENT_LOG_DSIM_CMD(struct v4l2_subdev *sd, u8 cmd_id, const u8 *cmd);
...
#define DISP_SS_EVENT_LOG_DSIM_CMD(...) do { } while(0) 
#define DISP_SS_EVENT_SHOW(...) do { } while(0)
#define DISP_SS_EVENT_SIZE_ERR_LOG(...) do { } while(0)
#endif


#define DECON_DISPLAY_ON_REQ	1
#define DECON_DISPLAY_ON_DONE 	0
...
#define WIN_UPDATE_BOTH		WIN_UPDATE_DSI0 | WIN_UPDATE_DSI1

#define DECON_NEED_UPDATE	0x80000000
#define DECON_ON			0x01000000
#define DECON_OFF			0x00000000
#define CHECK_DECON_ON(x)	(x & DECON_ON)


#define TE_NEED_UPDATE		0x00800000
#define TE_REF_MAIN			0x00010000
#define TE_REF_SUB			0x00020000
#define TE_OFF				0x00000000

#define DSIM0_NEED_UPDATE	0x00008000
#define DSIM0_MASTER		0x00000100
#define DSIM0_SLAVE			0x00000200
#define DSIM0_STANDALONE	0x00000300
#define DSIM0_OFF			0x00000000
#define GET_DSIM0_STAT(x) (x & 0x00000f00) >> 8 

#define DSIM1_NEED_UPDATE	0x00000080
#define DSIM1_MASTER		0x00000001
#define DSIM1_SLAVE			0x00000002
#define DSIM1_STANDALONE	0x00000003
#define DSIM1_OFF			0x00000000
#define GET_DSIM1_STAT(x) (x & 0x0000000f)

#define DSIM_MASTER			0x01
#define DSIM_SLAVE			0x02
#define DSIM_STANDALONE		0x03
#define DSIM_OFF			0x00
...
struct decon_device {
	...
	unsigned int need_disp_on;
	unsigned int irq_te_sub;
	unsigned int dual_dsi_stat;
};
...
int find_subdev_wb(struct decon_device *decon);
...
irqreturn_t decon_ext_wb_irq_handler(int irq, void *dev_data);
...
int decon_enable_lpd(struct decon_device *decon);
int decon_disable_lpd(struct decon_device *decon);
...
int __decon_exit_lpd(struct decon_device *decon);
int __decon_enter_lpd(struct decon_device *decon);
