	...
	const unsigned char *cmd, unsigned int cmd_size)
{
	...
	if (dsim->state != DSIM_STATE_HSCLKEN) {
		dev_err(dsim->dev, "DSIM ID : %d is not ready. state(%d)\n", dsim->id, dsim->state);
		ret = -EINVAL;
	...
	switch (id) {
		...
		case MIPI_DSI_DCS_READ:
			...
			if (dsim_wait_for_cmd_fifo_empty(dsim, 0)) {
				dev_err(dsim->dev, "ID:%d : MIPI DSIM ID : %d short packet write Timeout! 0x%x\n",
							id, dsim->id, cmd[0]);
				ret = -ETIMEDOUT;
				goto exit;
			}
		...
		case MIPI_DSI_DCS_LONG_WRITE:
			...
			dsim_reg_wr_tx_header(dsim->id, id, cmd_size & 0xff, (cmd_size & 0xff00) >> 8);
			
			if (dsim_wait_for_cmd_fifo_empty(dsim, 1)) {
				dev_err(dsim->dev, "ID: %d : MIPI DSIM ID : %d write Timeout!  0x%x\n",
							id, dsim->id, cmd[0]);
				ret = -ETIMEDOUT;
				goto exit;
			}
...
static int dsim_read_data(struct dsim_device *dsim, u8 addr, u8 count, u8 *buf)
{
	...
	dsim_write_command(dsim, MIPI_DSI_SET_MAXIMUM_RETURN_PACKET_SIZE, &count , 1);
	dsim_write_command(dsim, MIPI_DSI_DCS_READ, &addr, 1);
...
try_write:
	...
	if (ret != 0) {
		if (--retry)
			...
			dsim_err("dsim:%d write failed,  cmd : %x\n", dsim->id, cmd[0]);
	}
...
static int dsim_get_gpios(struct dsim_device *dsim)
{
	...
	if (of_get_property(dev->of_node, "gpios", NULL) != NULL)  {
		/* panel reset */
		res->lcd_rst_gpio = of_get_gpio(dev->of_node, 0);
		if (res->lcd_rst_gpio < 0) {
...
static int dsim_get_regulator(struct dsim_device *dsim)
{
	...
	int regulator_cnt = 0;
	char *regulator_str= NULL;
	struct device *dev = dsim->dev;
	struct dsim_resources *res = &dsim->res;

	dsim_info("%s +\n", __func__);
	
	regulator_cnt = of_property_count_strings(dev->of_node, "regulator_list");
	dsim_info("LCD Regulator Count : %d\n", regulator_cnt);

	for (i = 0; i < regulator_cnt; i++) {
		of_property_read_string_index(dev->of_node, "regulator_list", i, (const char **)&regulator_str);
		if (regulator_str != NULL) {
			dsim_info("getting string : %s\n",regulator_str);
			res->lcd_pwr[i].pwr_regulator = regulator_get(NULL, regulator_str);
			if (IS_ERR(res->lcd_pwr[i].pwr_regulator)) {
				dsim_err("%s : failed to regulator_get : %s\n", __func__, regulator_str);
				res->lcd_pwr[i].pwr_regulator = NULL;
				continue;
			}
			of_property_read_u32_index(dev->of_node, "on_off_delay_usec", cnt++, &value);
			dsim_info("regulator:%d on delay : %d\n", i, value);
			res->lcd_pwr[i].on_delay_usec = value;

			of_property_read_u32_index(dev->of_node, "on_off_delay_usec", cnt++, &value);
			dsim_info("regulator:%d off delay : %d\n",i, value);
			res->lcd_pwr[i].off_delay_usec= value;
		}
	}
	
	for (i = 0; i < regulator_cnt; i++) {
		if (res->lcd_pwr[i].pwr_regulator)
			ret = regulator_enable(res->lcd_pwr[i].pwr_regulator);
			if (ret) {
...
static int dsim_reset_panel(struct dsim_device *dsim)
{
	...
	ret = gpio_request_one(res->lcd_rst_gpio, GPIOF_OUT_INIT_HIGH, "lcd_reset");
	if (ret < 0) {
		dsim_err("failed to get LCD reset GPIO\n");
		return -EINVAL;
	}
	...
	gpio_set_value(res->lcd_rst_gpio, 0);

	usleep_range(500, 600);
	gpio_set_value(res->lcd_rst_gpio, 1);

	gpio_free(res->lcd_rst_gpio);
...
static int dsim_panel_power_on(struct dsim_device *dsim)
{
	...
	for (i = 0; i < 3; i++) {
		if (res->lcd_pwr[i].pwr_regulator != NULL) {
			ret = regulator_enable(res->lcd_pwr[i].pwr_regulator);
			if (ret) {
			...
			delay = res->lcd_pwr[i].on_delay_usec;
			if (delay)
...
static int dsim_panel_power_off(struct dsim_device *dsim)
{
	...
	ret = gpio_request_one(res->lcd_rst_gpio, GPIOF_OUT_INIT_LOW, "lcd_reset");
	if (ret < 0) {
	...
	for (i = 2; i >= 0; i--) {
		if (res->lcd_pwr[i].pwr_regulator != NULL) {
			ret = regulator_disable(res->lcd_pwr[i].pwr_regulator);
			if (ret) {
				dsim_err("%s : failed to regulator_disable : %d\n", __func__, i);
				return ret;
			}
			delay = res->lcd_pwr[i].off_delay_usec;
			if (delay)
				usleep_range(delay, delay);
		}
	}
...
static int dsim_probe(struct platform_device *pdev)
{
	...
	if (dsim->id == 0 && dsim_reg_is_hs_clk_ready(dsim->id)) {
