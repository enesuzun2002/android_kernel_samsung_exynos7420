...
extern int show_custom_logo(void);
#endif
...
#ifdef CONFIG_FOLDER_DUAL_PANEL
void decon_display_switch_by_hall(int hall_ic_state)
{
	struct decon_device *decon = decon_int_drvdata;
	struct esd_protect *esd;
	struct dsim_resources *res = &dsim0_for_decon->res;
	int lcd_sel_val;
	static int first_boot = 1;

	if (decon == NULL) {
		decon_err("%s decon is NULL\n", __func__);
		return;
	}

	decon_info("%s : state change: %d -> %d\n", __func__, decon->hall_ic_state, hall_ic_state);

	if (first_boot) {
		first_boot = 0;
		// check which panel is using in bootloader
		lcd_sel_val = gpio_get_value(res->lcd_sel);

		decon_info("%s lcd_sel gpio:%d\n", __func__, lcd_sel_val);

		if (lcd_sel_val == hall_ic_state) {
			decon_info("%s hall status is same as gpio, skip panel flip\n", __func__);
			decon->hall_ic_state = hall_ic_state;
			return;
		}
	} else {
		if (decon->hall_ic_state == hall_ic_state) {
			decon_info("%s hall status is same, skip panel flip\n", __func__);
			return;
		}
	}
	decon->hall_ic_state = hall_ic_state;
	...
	if (decon->state == DECON_STATE_OFF) {
		decon_info("%s decon state is off, skip panel flip\n", __func__);
		return;
	}
