...
{
	u32 val = (p & 0x3f) << 14 | (m & 0x3ff) << 4 | (s & 0x7) << 1;

	dsim_dbg("%s: p(%d), m(%d), s(%d), val(%d)\n", __func__, p, m, s, val);
...
static int dsim_reg_calculate_pms(struct dsim_pll_param *pll)
{
	...
	u32 fin = 24;
	u32 fout_min = 1500; /* 1.5 Gbps MIPI D-PHY */
		...
		for (m_div = 64; m_div <= 1023; m_div++) {
			for (s_div = 0; s_div <= 6; s_div++) {
				...
				if ((target_freq <= fout_cal) &&
					...
					/* When look up a correct value */
					if (target_freq == fout_min)
						break;
				}
			}
		}
