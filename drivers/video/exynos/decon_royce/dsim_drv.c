...
int dsim_write_hl_data(struct dsim_device *dsim, const u8 *cmd, u32 cmdSize)
{
	...
	if (panel->lcdConnected[get_panel_index_init()] == PANEL_DISCONNECTED)
		return cmdSize;
...
#if defined(CONFIG_DSIM_RX_FIFO_READ_DONE_IGNORE)
	int fifoctrl;
#endif
...
#if defined(CONFIG_DSIM_RX_FIFO_READ_DONE_IGNORE)
	/* for some panels, it can't find RX FIFO READ DONE FLAG
		so we judge Rx FIFO empty bit of DSIM_FIFOCTRL
		or skip reading */
	i = 0;
	while (i < DSIM_MAX_RX_FIFO) {
		fifoctrl = readl(dsim->reg_base + DSIM_FIFOCTRL);
		if (fifoctrl & DSIM_FIFOCTRL_RX_FIFO_EMPTY) {
			ret = rx_size;
			goto exit;
		}
		rx_fifo = readl(dsim->reg_base + DSIM_RXFIFO);
		pr_err("%s clear rx fifo : %08x, fifoctrl : %08x\n", 
			__func__, rx_fifo, fifoctrl);
		i++;
	}
	ret = 0;
	goto exit;
...
static int dsim_get_gpios(struct dsim_device *dsim)
{
	...
	/* panel selection */
	res->lcd_sel = of_get_named_gpio(dev->of_node, "lcd_sel", 0);
	if (!gpio_is_valid(res->lcd_sel)) {
		dsim_err("This board doesn't support lcd_sel GPIO");
	}
#endif
	dsim_info("%s -\n", __func__);
	return 0;
...
#ifdef CONFIG_FOLDER_DUAL_PANEL
static int dsim_select_panel(struct dsim_device *dsim)
{
	struct decon_device *decon = decon_int_drvdata;
	struct dsim_resources *res = &dsim->res;
	int ret;
	...
	ret = gpio_request_one(res->lcd_sel, GPIOF_DIR_OUT, "lcd_sel");
	if (ret < 0) {
		dsim_err("%s failed to get LCD_SEL GPIO\n",__func__);
		return -EINVAL;
	}

	//selection according to hall ic, 0:main LCD, 1:sub LCD
	val = decon->hall_ic_state ? 1 : 0;

	dsim_info("%s g_hall_ic_state: %d, set gpio as %d\n", __func__, decon->hall_ic_state, val);

	gpio_set_value(res->lcd_sel, val);
	gpio_free(res->lcd_sel);
	return 0;
}
#endif
