...
void decon_reg_set_mdnie_pclk(u32 id, u32 en)
{
	...
	decon_write_mask(id, ENHANCER_MIC_CTRL, val, ENHANCER_MIC_CTRL_MDNIE_PCLK_GATE);
...
void decon_reg_enable_mdnie(u32 id, u32 en)
{
	...
	decon_write_mask(id, ENHANCER_MIC_CTRL, val, ENHANCER_MIC_CTRL_MDNIE_ON_F);
}

void decon_reg_set_mdnie_blank(u32 id, u32 front, u32 sync, u32 back, u32 line)
{
	u32 val = MDNIE_FRM_BLANK_FRONT_F(front) | MDNIE_FRM_BLANK_SYNC_F(sync) |
		MDNIE_FRM_BLANK_BACK_F(back);
	decon_write(id, MDNIE_FRM_BLANK, val);
	decon_write(id, MDNIE_LINE_BLANK, MDNIE_LINE_BLANK_F(line));
}
