...
static void print_mc_state(struct modem_ctl *mc)
{
	...
	int cp_dump_int = gpio_get_value(mc->gpio_cp_dump_int);

	mif_info("%s: %pf: MC state:%s "
		"on:%d reset:%d active:%d status:%d cp_dump_int:%d reason:%d\n",
...
static irqreturn_t cp_active_handler(int irq, void *arg)
{
	...
	if (gpio_get_value(mc->gpio_phone_active) != 0) {
		...
		} else if (gpio_get_value(mc->gpio_cp_dump_int) == 0) {
