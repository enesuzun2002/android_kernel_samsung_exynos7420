		...
		spinlock_t *lock)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (!req) {
			pr_info("usb:(hsic) %s: req allocated:%d\n", __func__, i);
...
static void ghsic_data_unthrottle_tx(void *ctx)
{
	...
	pr_debug("usb:(hsic) %s: port num =%d unthrottled\n", __func__,
		port->port_num);
}
...
static void ghsic_data_write_tohost(struct work_struct *w)
{
	...
	while (!list_empty(&port->tx_idle)) {
		...
		spin_lock_irqsave(&port->tx_lock, flags);
		if (ret) {
			pr_info("usb:(hsic) %s: usb epIn failed\n", __func__);
		...
		if (ghsic_data_fctrl_support &&
			...
			pr_debug("usb:(hsic) %s: disable flow ctrl:"
					" tx skbq len: %u\n",
...
static int ghsic_data_receive(void *p, void *data, size_t len)
{
	...
	if (ghsic_data_fctrl_support &&
		...
		pr_info("usb:(hsic) %s: flow ctrl enabled: tx skbq len: %u\n",
					__func__, port->tx_skb_q.qlen);
		spin_unlock_irqrestore(&port->tx_lock, flags);
...
static void ghsic_data_write_tomdm(struct work_struct *w)
{
	...
	while ((skb = __skb_dequeue(&port->rx_skb_q))) {
		...
		if (ret < 0) {
			...
			pr_info("usb:(hsic) %s: write error:%d\n",
					__func__, ret);
...
static void ghsic_data_epin_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	case -ESHUTDOWN:
		...
		return;
	default:
		pr_info("usb:(hsic) %s: data tx ep error %d\n", __func__, status);
		break;
	}
...
ghsic_data_epout_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	case -ESHUTDOWN:
		...
		return;
	default:
		pr_info("usb:(hsic) %s: %s response error %d, %d/%d\n",
...
static void ghsic_data_start_rx(struct gdata_port *port)
{
	...
	while (atomic_read(&port->connected) && !list_empty(&port->rx_idle)) {
		...
		if (ret) {
			...
			pr_info("usb:(hsic) %s: rx queue failed\n", __func__);
...
static void ghsic_data_connect_w(struct work_struct *w)
{
	...
	if (ret) {
		pr_info("usb:(hsic) %s: unable open bridge ch:%d err:%d\n",
				__func__, port->brdg.ch_id, ret);
		return;
	}

	pr_info("usb:(hsic) %s : set bit - CH_OPENED (sts=%ld)\n", __func__, port->bridge_sts);
	set_bit(CH_OPENED, &port->bridge_sts);
	pr_info("usb:(hsic) %s : result (sts=%ld)\n", __func__, port->bridge_sts);
...
static void ghsic_data_disconnect_w(struct work_struct *w)
{
	...
	pr_info("usb:(hsic) %s Clear CH_OPENED \n", __func__);
...
static int ghsic_data_probe(struct platform_device *pdev)
{
	...
	pr_info("usb:(hsic) %s: name:%s no_data_ports= %d\n",
...
#ifdef CONFIG_OF
static const struct of_device_id usb_hsic_data_bridge_dt_ids[] = {
	{ .compatible = "samsung,acm_data_mdm_bridge",
	},
	{ },
};
MODULE_DEVICE_TABLE(of, usb_hsic_data_bridge_dt_ids);
#endif
...
#ifdef CONFIG_OF
	pdrv->driver.of_match_table	= of_match_ptr(usb_hsic_data_bridge_dt_ids);
#endif
...
void ghsic_data_disconnect(void *gptr, int port_num)
{
	...
	pr_info("usb:(hsic) %s: port#%d\n", __func__, port_num);

	port = gdata_ports[port_num].port;

	if (port_num > no_data_ports) {
		pr_info("usb:(hsic) %s: invalid portno#%d\n", __func__, port_num);
		return;
	}

	if (!gptr || !port) {
		pr_info("usb:(hsic) %s: port is null\n", __func__);
		return;
	}
...
int ghsic_data_setup(unsigned num_ports, enum gadget_type gtype)
{
	...
	for (i = first_port_id; i < (num_ports + first_port_id); i++) {
		...
		if (ret) {
			no_data_ports--;
			pr_debug("usb:(hsic) %s: Unable to alloc port:%d\n", __func__, i);
			goto free_ports;
		}
	}
