...
static unsigned ch1FloatSat = 65535;
static unsigned ch2FloatSat = 65535;
static unsigned ch3FloatSat = 65535;
static unsigned ch4FloatSat = 65535;
...
adpd143_mode_switching(struct adpd143_data *pst_adpd, u32 usr_mode)
{
	...
	case SAMPLE_USR:
		...
		atomic_set(&pst_adpd->hrm_enabled, 1);
		if (is_Prev_TIA_ADC) {
	...
	case TIA_ADC_USR:
		...
		/* for TIA_ADC runtime Dark Calibration */
		ch1FloatSat = ch2FloatSat = ch3FloatSat = ch4FloatSat = 65535;
	...
	case S_SAMP_XY_A:
		...
		if(GET_USR_MODE(usr_mode) != TIA_ADC_USR) {
			...
			for (cnt = 0; cnt < pst_adpd->fifo_size; cnt += 4) {
				...
				pr_err("adpd143 SLOT A(TIA) unch3:%d, ch4=%d\n",
	...
	case S_SAMP_XY_AB:
		...
		if (GET_USR_MODE(usr_mode) != TIA_ADC_USR) {
			for (cnt = 0; cnt < pst_adpd->fifo_size; cnt += 8) {
				...
				if (atomic_read(&pst_adpd->hrmled_enabled)) {
					...
					input_event(pst_adpd->hrm_input_dev,
						EV_REL, REL_Y, sum_slot_b + 1);
					input_sync(pst_adpd->hrm_input_dev);
					pr_err("adpd143 sum_slot_a:%d, sum_slot_b:%d\n",
			...
			for (cnt = 0; cnt < pst_adpd->fifo_size; cnt += 8) {
				...
				/* for making float mode saturated at the point */
				for (ch = 4; ch < 8; ++ch) {
					pr_err("adpd142 SLOT B(float mode) : rawDataCh%d : %d, rawDarkCh%d : %d\n",
					...
					if ((int)rawDataCh[ch] >=
						...
						rawDataCh[ch] =
							FLOAT_STR_CONTROL - rawDarkCh[ch];
						if (ch == 4 &&
							(((rawDataCh[0] < ch1FloatSat)
							&& (rawDataCh[0] > ch1FloatSat - 20))
							|| (ch1FloatSat == 65535)))
							ch1FloatSat = rawDataCh[0];
						if (ch == 5 &&
							(((rawDataCh[1] < ch2FloatSat)
							&& (rawDataCh[1] > ch2FloatSat - 20))
							|| (ch2FloatSat == 65535)))
							ch2FloatSat = rawDataCh[1];
						if (ch == 6 &&
							(((rawDataCh[2] < ch3FloatSat)
							&& (rawDataCh[2] > ch3FloatSat - 20))
							|| (ch3FloatSat == 65535)))
							ch3FloatSat = rawDataCh[2];
						if (ch == 7 &&
							(((rawDataCh[3] < ch4FloatSat)
							&& (rawDataCh[3] > ch4FloatSat - 10))
							|| (ch4FloatSat == 65535)))
				...
				for (ch = 4; ch < 8; ++ch) {
					if (GET_USR_MODE(usr_mode) == TIA_ADC_USR) {
						...
						input_event(pst_adpd->hrm_input_dev,
							EV_MSC, MSC_RAW, rawDataCh[ch]);
						pr_err("adpd143 SLOT B(After Dark Cal) %d:%d\n",
							ch, rawDataCh[ch]);
					}
				}
				if (uncoated_ch3 == 0) {
	...
	if (pst_adpd->pre_hrmled_enable != val) {
		switch(val) {
			case IRRED_OFF:
				pr_info("%s: IRRED_OFF.\n", __func__);
				cmd_parsing("0x0", 1, parse_data);
				atomic_set(&pst_adpd->hrmled_enabled, 0);
				break;
			case IR_MODE:
				pr_info("%s: IR_MODE.\n", __func__);
				cmd_parsing("0x32", 1, parse_data);
				atomic_set(&pst_adpd->hrmled_enabled, 1);
				break;
			case RED_MODE:
				pr_info("%s: RED_MODE.\n", __func__);
