	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (data->part_type < PART_TYPE_MAX86902A) { //86900
		...
		if (new_value) { //alc on
			...
			err = max86900_write_reg(data, 0x8e, 0x00);
			if (err != 0) {
				pr_err("%s - error initializing 0x8e!\n",
					__func__);
				return -EIO;
			...
			err = max86900_write_reg(data, MAX86902_LED1_PA, 0xff);
			if (err != 0) {
				pr_err("%s - error initializing MAX86902_LED1_PA!\n",
					__func__);
				return -EIO;
			...
			err = max86900_write_reg(data, MAX86902_LED3_PA, 0xff);
			if (err != 0) {
				pr_err("%s - error initializing MAX86902_LED3_PA!\n",
					__func__);
				return -EIO;
			...
			err = max86900_write_reg(data, MAX86902_LED4_PA, 0xff);
			if (err != 0) {
				pr_err("%s - error initializing MAX86902_LED4_PA!\n",
					__func__);
				return -EIO;
