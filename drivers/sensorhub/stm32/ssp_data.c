		...
		u16 mode, int type)
{
	...
	if (mode == BATCH_MODE_RUN)
		data->latest_timestamp[type] += ts_delta_ns;
	else
		data->latest_timestamp[type] = data->timestamp;
	/* Ignore MCU's timestamp
	event->timestamp = data->latest_timestamp[sensor];
	*/
