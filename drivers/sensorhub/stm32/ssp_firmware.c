...
static int load_ums_fw_bootmode(struct spi_device *spi, const char *pFn)
{
	...
	while (remaining > 0) {
		...
		while (retry_count < 3) {
			ret = fw_write_stm(spi, fw_addr, block, buff);
			if (ret < block) {
				ssp_err("Err writing to addr 0x%08X", fw_addr);
				if (ret < 0) {
					ssp_err("Error was %d", ret);
				} else {
...
static int load_kernel_fw_bootmode(struct spi_device *spi, const char *pFn)
{
	...
	while (remaining > 0) {
		...
		while (retry_count < 3) {
			ret = fw_write_stm(spi, fw_addr, block, fw->data+uPos);
			if (ret < block) {
				ssp_err("Err writing to addr 0x%08X", fw_addr);
				if (ret < 0) {
					ssp_err("Error was %d", ret);
...
static int update_mcu_bin(struct ssp_data *data, int iBinType)
{
	...
	do {
		...
		ssp_info("bootmode %d retry: %d", ret, 3 - retry);
	} while (retry-- > 0 && ret != BL_ACK);

	if (ret != BL_ACK) {
		ssp_errf("change_to_bootmode %d", ret);
		return ret;
	}
	...
	/* HW request: I2C line is reversed */
		ret = load_kernel_fw_bootmode(data->spi, BL_FW_NAME);
		break;
	case KERNEL_CRASHED_BINARY:
		ret = load_kernel_fw_bootmode(data->spi, BL_CRASHED_FW_NAME);
		break;
	case UMS_BINARY:
		ret = load_ums_fw_bootmode(data->spi, BL_UMS_FW_NAME);
		break;
	default:
...
int forced_to_download_binary(struct ssp_data *data, int iBinType)
{
	...
	do {
		...
		ret = update_mcu_bin(data, iBinType);
	} while (retry-- > 0 && ret < 0);
...
int check_fwbl(struct ssp_data *data)
{
	...
	if ((data->curr_fw_rev == SSP_INVALID_REVISION)
			|| (data->curr_fw_rev == SSP_INVALID_REVISION2)) {
		data->curr_fw_rev = SSP_INVALID_REVISION;
		ssp_err("SSP_INVALID_REVISION");
		return FW_DL_STATE_NEED_TO_SCHEDULE;
	} else {
		if (data->curr_fw_rev != fw_revision) {
			ssp_info("MCU Firm Rev : Old = %8u, New = %8u",
				data->curr_fw_rev, fw_revision);
