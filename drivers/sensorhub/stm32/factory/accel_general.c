...
static ssize_t accel_selftest(char *buf, struct ssp_data *data)
{
	...
	diff_min = ((s16)(temp[8] << 8)) + temp[7];
	diff_max = ((s16)(temp[10] << 8)) + temp[9];
	result = temp[11];

	pr_info("[SSP] %s - %d, %d, %d, %d, %d, %d, %d\n", __func__,
		init_status, result, diff_axis[0], diff_axis[1], diff_axis[2], diff_min, diff_max);
	return sprintf(buf, "%d,%d,%d,%d\n",
