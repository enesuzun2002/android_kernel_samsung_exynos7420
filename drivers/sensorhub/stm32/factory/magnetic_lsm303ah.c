...
exit:
#if 0 /* This is draft version of result */
	return sprintf(buf,
			"%d, %d, %d, %d, %d, %d, %d, %d, %d, %d \n",
			err[0], id, err[1], x_diff, err[2], y_diff, err[3], z_diff, err[4], result);
#endif
#if 1 /* This is result format requested by SystemSW team. */
	return sprintf(buf,
			"%d, %d, %d, %d \n",
...
int magetic_chip_id_read(struct ssp_data *data)
{
	int ret = 0;
	char chip_id = 0;
	...
	msg->cmd = SH_MSG2SSP_GET_MAGNETIC_STATUS;
	msg->length = 1;
	msg->options = AP2HUB_READ;
	msg->buffer = &chip_id;
	msg->free_buffer = 0;
	...
	pr_info("[SSP] magetic_chip_id_read chip_id 0x%x \n", data->chip_id);
		...
		struct device_attribute *attr, char *buf)
{
	...
	pr_info("[SSP] magnetic chip_id 0x%x \n", data->chip_id);
