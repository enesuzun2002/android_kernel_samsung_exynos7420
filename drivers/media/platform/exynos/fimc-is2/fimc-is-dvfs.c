...
DECLARE_DVFS_CHK_FUNC(FIMC_IS_SN_PREVIEW_HIGH_SPEED_FPS)
{
	...
	bool setfile_flag = ((mask == ISS_SUB_SCENARIO_STILL_PREVIEW) ||
			(mask == ISS_SUB_SCENARIO_STILL_PREVIEW_WDR_ON) ||
			(mask == ISS_SUB_SCENARIO_STILL_PREVIEW_WDR_AUTO) ||
			(mask == ISS_SUB_SCENARIO_FRONT_PANORAMA));

	if((fimc_is_sensor_g_framerate(device->sensor) > 30) && setfile_flag)
		return 1;
	else
		return 0;
}
...
DECLARE_DVFS_CHK_FUNC(FIMC_IS_SN_FRONT_CAMCORDING)
{
	...
	if ((fimc_is_sensor_g_postion(device->sensor) == SENSOR_POSITION_FRONT) &&
		...
		setfile_flag && (device->scp.output.width < 2560))
		return 1;
	else
		return 0;
}
...
DECLARE_DVFS_CHK_FUNC(FIMC_IS_SN_FRONT_CAPTURE)
{
	...
	if ((fimc_is_sensor_g_postion(device->sensor) == SENSOR_POSITION_FRONT) &&
		...
		(!(test_bit(FIMC_IS_COMPANION_OPEN, &core->companion.state))) &&
		((static_ctrl->cur_scenario_id >= FIMC_IS_SN_FRONT_PREVIEW)
		    && (static_ctrl->cur_scenario_id <= FIMC_IS_SN_FRONT_PREVIEW))
		)
		return 1;
	else
...
DECLARE_DVFS_CHK_FUNC(FIMC_IS_SN_FRONT_COMPANION_CAPTURE)
{
	...
	if ((fimc_is_sensor_g_postion(device->sensor) == SENSOR_POSITION_FRONT) &&
		(test_bit(FIMC_IS_COMPANION_OPEN, &core->companion.state)) &&
		(test_bit(FIMC_IS_ISCHAIN_REPROCESSING, &device->state)) &&
		((static_ctrl->cur_scenario_id >= FIMC_IS_SN_FRONT_COMPANION_PREVIEW)
		    && (static_ctrl->cur_scenario_id <= FIMC_IS_SN_FRONT_COMPANION_PREVIEW))
		)
		return 1;
	else
...
DECLARE_DVFS_CHK_FUNC(FIMC_IS_SN_FRONT_COMPANION_CAMCORDING_CAPTURE)
{
	...
	if ((fimc_is_sensor_g_postion(device->sensor) == SENSOR_POSITION_FRONT) &&
		(test_bit(FIMC_IS_ISCHAIN_REPROCESSING, &device->state)) &&
		(test_bit(FIMC_IS_COMPANION_OPEN, &core->companion.state)) &&
		((static_ctrl->cur_scenario_id >= FIMC_IS_SN_FRONT_COMPANION_CAMCORDING)
		    && (static_ctrl->cur_scenario_id <= FIMC_IS_SN_FRONT_COMPANION_CAMCORDING_WHD_CAPTURE)) &&
		(device->scp.output.width < 2560)
		)
		return 1;
	else
...
DECLARE_DVFS_CHK_FUNC(FIMC_IS_SN_REAR_CAPTURE)
{
	...
	if ((fimc_is_sensor_g_postion(device->sensor) == SENSOR_POSITION_REAR) &&
		test_bit(FIMC_IS_ISCHAIN_REPROCESSING, &device->state) &&
		((static_ctrl->cur_scenario_id >= FIMC_IS_SN_REAR_PREVIEW_FHD)
		    && (static_ctrl->cur_scenario_id <= FIMC_IS_SN_REAR_PREVIEW_UHD))
		)
		return 1;
	else
		return 0;
