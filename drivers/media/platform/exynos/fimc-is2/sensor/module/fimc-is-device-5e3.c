...
static unsigned int front_hw_rev;
	...
	struct exynos_platform_fimc_is_module *pdata)
{
	...
	of_property_read_u32(dnode, "rom_power_seq_ver", &rom_power_seq_ver);
	pr_info("%s rom_power_seq_ver:%d\n", __func__, rom_power_seq_ver);
	...
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_reset, "rst_high", PIN_OUTPUT, 1, 1000);
...
int sensor_5e3_probe(struct platform_device *pdev)
{
	...
	if (fimc_is_dev == NULL) {
		warn("fimc_is_dev is not yet probed(5e3)");
	...
	ext->sensor_con.product_name = SENSOR_S5K5E3_NAME;
...
MODULE_DEVICE_TABLE(of, exynos_fimc_is_sensor_5e3_match);
...
{
	int ret = platform_driver_register(&sensor_5e3_driver);
	if (ret)
