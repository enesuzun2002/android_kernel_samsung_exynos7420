	...
	u32 load_addr, char *setfile_name)
{
	...
	if (IS_ERR_OR_NULL(fp)) {
#ifdef CONFIG_USE_VENDER_FEATURE
		if (is_dumped_fw_loading_needed && core->current_position == SENSOR_POSITION_REAR) {
			memset(setfile_path, 0x00, sizeof(setfile_path));
			snprintf(setfile_path, sizeof(setfile_path), "%s%s",
...
#ifdef CONFIG_USE_VENDER_FEATURE
		if (!test_bit(FIMC_IS_COMPANION_S_INPUT, &core->companion.state)) {
...
{
	int ret = 0;
	struct camera2_sm *capability = NULL;
...
#ifdef CONFIG_COMPANION_USE
	struct fimc_is_core *core = (struct fimc_is_core *)device->resourcemgr->private_data;
	struct exynos_platform_fimc_is_companion *comp_pdata = core->companion.pdata;
#endif
	BUG_ON(!device);
...
#ifdef CONFIG_COMPANION_USE
	if (atomic_read(&device->resourcemgr->resource_ischain.rsccount) == 0) {
		if (core->spi0.device) {
		...
		pin_config_set(comp_pdata-> comp_int_pinctrl, comp_pdata->comp_int_pin,
...
#else
			ret = fimc_is_ischain_loadcalb(device, NULL, SENSOR_POSITION_REAR);
#endif
			if (ret) {
...
static int fimc_is_ischain_start(struct fimc_is_device_ischain *device)
{
	...
	/* Fade In/Out */
	device->is_region->parameter.TaAlg.ParameterAeConfig.uiCameraEntrance = device->sensor->fade_in_mode;
	...
	u32 *indexes)
{
	...
	dma_output = fimc_is_itf_g_param(device, frame, PARAM_3AA_VDMA2_OUTPUT);
	/* CAUTION : can't make both otf and dma disable together */
	...
	struct fimc_is_frame *check_frame)
{
	...
	if (test_bit(FIMC_IS_GROUP_OTF_INPUT, &group->state)) {
		...
		vendor_afstate_bk = frame->shot->ctrl.aa.vendor_afState;
		...
		frame->shot->ctrl.aa.vendor_afState = vendor_afstate_bk;
	}
#endif
