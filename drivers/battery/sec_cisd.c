...
bool sec_bat_cisd_check(struct sec_battery_info *battery)
{
	...
	if ((battery->status == POWER_SUPPLY_STATUS_CHARGING) ||
		...
		pcisd->overflow_cap_thr = capcurr_val.intval > battery->pdata->cisd_cap_limit ? capcurr_val.intval : battery->pdata->cisd_cap_limit;
		pcisd->overflow_start_time = 0;
	...
	pr_info("cisd_debug: %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n",
...
void sec_battery_cisd_init(struct sec_battery_info *battery)
{
	...
	battery->cisd.overflow_cap_thr = capfull_val.intval > battery->pdata->cisd_cap_limit ? capfull_val.intval : battery->pdata->cisd_cap_limit;
