...
static LIST_HEAD(epd_list);
static DEFINE_MUTEX(epd_list_lock);
...
{
	struct exynos_pm_domain *exypd = NULL, *epd;

	if (IS_ERR_OR_NULL(domain_name))
		return NULL;

	mutex_lock(&epd_list_lock);
	list_for_each_entry(epd, &epd_list, epd_list_node) {
		if (!strcmp(epd->name, domain_name)) {
			exypd = epd;
			break;
		}
	}
	mutex_unlock(&epd_list_lock);
	return exypd;
}
...
static int exynos_pd_status(struct exynos_pm_domain *pd)
{
	...
	if (likely(pd->base)) {
		...
		pd->status = (__raw_readl(pd->base+0x4) & LOCAL_PWR_CFG);
	}
...
static int exynos_pd_power(struct exynos_pm_domain *pd, int power_flags)
{
	...
	if (likely(pd->base)) {
		...
		if (!strcmp(pd->name, "pd-g3d"))
			__raw_writel(0xFFFF00C0, pd->base+0x10);
#endif

		/* on/off value to CONFIGURATION register */
		__raw_writel(power_flags, pd->base);
#if  defined(CONFIG_SOC_EXYNOS5433) ||  defined(CONFIG_SOC_EXYNOS7580)
		/* Wait max 1ms */
		timeout = 100;
#endif
...
static int exynos_genpd_power_off(struct generic_pm_domain *genpd)
{
	...
	if (unlikely(ret)) {
		if (ret == -EBUSY) {
			...
			if (pd->cb && pd->cb->forced_off_pre)
				pd->cb->forced_off_pre(pd);
			ret = pd->off(pd, 0);
			if (unlikely(ret)) {
...
static __init int exynos_pm_dt_parse_domains(void)
{
	...
	for_each_compatible_node(np, NULL, "samsung,exynos-pd") {
		...
		pd->pd_data = (void *)exynos_pd_find_data(pd);
#endif
		...
		if (ret)
			...
			pd->pd_option = val;
#if defined(CONFIG_SOC_EXYNOS7420)
		ret = of_property_read_u32_index(np, "pd-timeout", 0, &val);
		if (ret)
			pd->pd_timeout = 1;
		else
			pd->pd_timeout = val;
		...
		mutex_lock(&epd_list_lock);
		list_add(&pd->epd_list_node, &epd_list);
		mutex_unlock(&epd_list_lock);
...
subsys_initcall(exynos5_pm_domain_init);
