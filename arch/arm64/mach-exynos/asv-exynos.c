...
static int asv_group_show(struct seq_file *s, void *d)
{
	...
	list_for_each_entry(ai, &asv_list, node) {
		...
		if (ai->ops_cal && ai->ops_cal->get_use_abb)
		...
		for (i = 0; i < ai->dvfs_level_nr; i++) {
			...
			if (useABB)
				seq_printf(s, "%s LV%d freq : %d volt : %d, "
						...
						ai->asv_rcc[i].asv_value,
						ai->asv_sub_grp[i].asv_grp);
			else
				seq_printf(s, "%s LV%d freq : %d volt : %d, "
						"rcc : %d, group : %d\n",
						ai->name, i,
...
{
	unsigned int asv_summary[ASV_TYPE_END];
	unsigned int i;

	for (i = 0; i < ASV_TYPE_END; i++)
		asv_summary[i] = get_sub_grp_match_asv_grp(i, 0);

	seq_printf(s, "big:%u, LITTLE:%u, INT:%u, MIF:%u, G3D:%u, ISP:%u\n",
			asv_summary[ID_CL1], asv_summary[ID_CL0],
			asv_summary[ID_INT], asv_summary[ID_MIF],
			asv_summary[ID_G3D], asv_summary[ID_ISP]);
	return 0;
}
